...
static bool serializeDetachToImmediateSync(BasicBlock *BB) {
  ...
  if (isa<SyncInst>(I)) {
    ...
    for (DetachInst *DI : DetachPreds) {
      assert(!isa<SDetachInst>(DI) && "sdetach is immediately synced?!");
    ...
    for (Instruction *RI : ReattachPreds) {
      assert(!isa<SReattachInst>(RI) && "sreattach is immediately synced?!");
...
/// If BB reattaches and is the tail of a chain of detached blocks
/// whose instructions have no side effects, serialize the reattach and detach.
/// Return true iff those conditions are met.
/// TODO(mcj) handle an arbitrary predecessor graph
static bool serializeNoSideEffectsDetachedChain(BasicBlock *BB) {
  SReattachInst *RI = dyn_cast<SReattachInst>(BB->getTerminator());
  if (!RI) return false;

  // The following is predicated on ReattachInst not having side effects. If
  // that is not true, then this needs to be reworked
  assert(!RI->mayHaveSideEffects());

  // Find the detach corresponding to RI, only if it is along a chain of blocks
  // whose instructions have no side effects.
  ...
  while (!DI) {
    ...
    TerminatorInst *TI = PredBB->getTerminator();
    DI = dyn_cast<SDetachInst>(TI);
    if (DI) {
      // If we find a detach instruction that doesn't detach into this block,
      // then I guess BB is the detach's continue? This situation is scary, bail
      if (DI->getDetached() != BB) return false;
    } else if (BranchInst *BI = dyn_cast<BranchInst>(TI)) {
      if (BI->isConditional()) return false;
    } else if (isa<SReattachInst>(TI)) {
      // If we encounter another reattach along the chain, certainly bail
      return false;
    } else {
      // TODO(mcj) while the SReattachInst case above is a special case of this one,
      // we might want to later reason about safe non-branches
      return false;
    }
    BB = PredBB;
  }
  DEBUG(dbgs() << "Serialize detached chain of blocks whose instructions "
                  "have no side effects between\n"
               << *DI << "\nand\n" << *RI << "\n");
  assert(DI->getFunction() == RI->getFunction());
  BasicBlock *Continue = DI->getContinue();
  assert(Continue == RI->getSuccessor(0) &&
        "Reattach destination does not match continue block of detach.");
  assert(Continue->phis().begin() == Continue->phis().end() &&
         "Continue block has phi nodes");

  // Serialize the detach and reattach: replace them with unconditional branches
  ...
  if (ReattachInst *RI = dyn_cast<ReattachInst>(I)) {
    ...
    for (BasicBlock *PredBB : predecessors(BB)) {
      ...
      if (!DI || DI->getDetached() != BB) {
        DEBUG(dbgs() << "Not serializing due to predecessor " << *PredBB
                     << " that does not detach to " << *BB << "\n");
        return false;
      }
    }
    ...
    for (BasicBlock *PredBB : predecessors(BB)) {
      ...
      DEBUG(dbgs() << "Serializing detached instruction\n  " << *DI << "\n");
      BasicBlock *Detached = DI->getDetached();
      BasicBlock *Continue = DI->getContinue();
...
bool llvm::isExpandableLoopLatch(const BranchInst *BI) {
  const BasicBlock *Latch = BI->getParent();

  if (std::distance(pred_begin(Latch), pred_end(Latch)) != 2) {
    DEBUG(dbgs() << "latch does not have exactly two predecessors\n");
    return false;
  }
  ...
  const BasicBlock *Header =
      isa<SDetachInst>(Pred1->getTerminator()) ? Pred1 : Pred2;
  const SDetachInst *DI = dyn_cast<const SDetachInst>(Header->getTerminator());
  const BasicBlock *ReattachBlock = (Pred1 == Header) ? Pred2 : Pred1;
  const SReattachInst *RI =
      dyn_cast<const SReattachInst>(ReattachBlock->getTerminator());
  if (!DI || !RI) {
    DEBUG(dbgs() << "Latch predecessors are not detach and reattach.\n");
    return false;
  }
  ...
  if (RI->getDetachContinue() != Latch) {
    DEBUG(dbgs() << "Continuation of reattach is not the latch.\n");
    return false;
  }
  if (!is_contained(successors(Latch), Header)) {
    DEBUG(dbgs() << "Does not branch back to the detacher.\n");
    return false;
  }
...
bool SimplifyCFGOpt::run(BasicBlock *BB) {
  ...
  Changed |= serializeNoSideEffectsDetachedChain(BB);
  ...
  if (BranchInst *BI = dyn_cast<BranchInst>(BB->getTerminator())) {
    if (BI->isUnconditional()) {
     ...
     // [victory] I observed SimplifyBranchOnICmpChain causing an expandable
     // loop latch terminating branch to be replaced with a switch, which
     // caused issues, as LoopCoarsen and LoopExpansion expect loops latches to
     // terminate in a conditional branch.  Let's simply disable optimizations
     // that rewrite conditional branches that terminate expandable loop
     // latches: any potential simplification can wait until a later run of
     // SimplifyCFG after the loop is expanded.
     if (!isExpandableLoopLatch(BI))
