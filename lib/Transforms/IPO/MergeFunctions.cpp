...
#include "../Swarm/Utils/SwarmABI.h"
...
private:
  ...
  /// Replace all direct Swarm enqueues of Old with Swarm enqueues of New.
  void replaceDirectEnqueuers(Function *Old, Function *New);
...
// Replace direct Swarm enqueuers of Old with New.
void MergeFunctions::replaceDirectEnqueuers(Function *Old, Function *New) {
  SmallVector<Use *, 8> InstUses;
  for (auto UI = Old->use_begin(), UE = Old->use_end(); UI != UE;) {
    Use *U = &*(UI++);
    if (isa<Instruction>(U->getUser())) {
      InstUses.push_back(U);
    } else if (auto *FunctionPtr = dyn_cast<ConstantExpr>(U->getUser())) {
      // Old may be used by a ConstantExpr that bitcasts the function pointer
      // to a different (pointer or integer) type.
      // This ConstantExpr in turn is Used by the actual enqueue CallInst.
      if (FunctionPtr->isCast()) {
        auto FPUI = FunctionPtr->use_begin(), FPUE = FunctionPtr->use_end();
        while (FPUI != FPUE) {
          Use *FPU = &*(FPUI++);
          if (isa<Instruction>(FPU->getUser()))
            InstUses.push_back(FPU);
        }
      }
    }
  }
  for (Use *U : InstUses) {
    auto *EnqueueI = cast<Instruction>(U->getUser());
    if (swarm_abi::getEnqueueFunction(EnqueueI) == Old) {
      DEBUG(dbgs() << "Merging Swarm enqueue task functions. Before:\n  "
                   << *EnqueueI
                   << "\n  After:\n  ");
      U->set(ConstantExpr::getPointerCast(New, U->get()->getType()));
      DEBUG(dbgs() << *EnqueueI << "\n");
      remove(EnqueueI->getParent()->getParent());
      assert(swarm_abi::getEnqueueFunction(EnqueueI) == New);
    }
  }
}
