...
void PassManagerBuilder::populateModulePassManager(legacy::PassManagerBase& MPM) {
  // Run initial optimizations minus loop unrolling, vectorization, and partial
  // load redundancy elimination.
  // We'll run all of them again after the Swarm passes.
  // We must run the same initial optimizations regardless of whether we're
  // autoparallelizing, to ensure consistent instrumentation (e.g., heartbeats).
  {
    auto OrigDisableGVNLoadPRE = DisableGVNLoadPRE;
    auto OrigDisableUnrollLoops = DisableUnrollLoops;
    auto OrigLoopVectorize = LoopVectorize;
    auto OrigSLPVectorize = SLPVectorize;
    // Disable Global Value Numbering's redundant load elimination to provide
    // the Fractalization pass with loops with fewer, simpler header PHINodes.
    DisableGVNLoadPRE = true;
    DisableUnrollLoops = true;
    LoopVectorize = false;
    SLPVectorize = false;
    prepopulateModulePassManager(MPM);
    DisableGVNLoadPRE = OrigDisableGVNLoadPRE;
    DisableUnrollLoops = OrigDisableUnrollLoops;
    LoopVectorize = OrigLoopVectorize;
    SLPVectorize = OrigSLPVectorize;
  }
  // Now do instrumentation that must run after initial serial optimizations.
  MPM.add(createBarrierNoopPass());
  MPM.add(createInsertHeartbeatsPass());
  MPM.add(createBarrierNoopPass());
  ...
  if (ParallelLevel != 0) {
    switch (ParallelLevel) {
      ...
      case 4: // fswarm
        ...
        break;
      case 3: //fdetach
        llvm_unreachable("SCC does not support -fdetach");
    ...
    if (OptLevel > 0) {
      ...
      if (ParallelLevel != 4) {
        ...
        // [mcj] Curiously we still see a lot of dead loops before entering the
        // Swarm passes.
        MPM.add(createLoopDeletionPass());
        MPM.add(createCFGSimplificationPass());
        MPM.add(createUnifyFunctionExitNodesPass());
        MPM.add(createCreateParallelizableCopyPass());
        MPM.add(createProfitabilityPass());
        MPM.add(createBundlingPass());
        MPM.add(createFractalizationPass());
        // Sometimes ScalarEvolution is now able to identify induction
        // variables or the tripcount of some loop.  IndVarSimplify rewrites
        // loops using this information, and then we clean up dead loads (e.g.
        // in the loop latch) that may no longer be needed.
        MPM.add(createIndVarSimplifyPass());
        MPM.add(createAggressiveDCEPass());
        MPM.add(createCFGSimplificationPass());
        // Clean up phis from LCSSA (required by Fractalization and
        // IndVarSimplify) to avoid bloat in shared loop closures.
        addInstructionCombiningPass(MPM);
        addExtensionsToPM(EP_Peephole, MPM);
        MPM.add(createLoopCoarsenPass());
        // LoopCoarsen adds much computation of loop limits and exit conditions
        // and leaves behind old dead computation.  Clean up before dead values
        // are allocated into closures by LoopExpansion.
        MPM.add(createAggressiveDCEPass());
        MPM.add(createCFGSimplificationPass());
        MPM.add(createLoopExpansionPass());

        // Cleanup mess generated by coarsening and expansion of loops
        MPM.add(createCFGSimplificationPass());
        addInstructionCombiningPass(MPM);
        addExtensionsToPM(EP_Peephole, MPM);
        MPM.add(NewGVN ? createNewGVNPass() : createGVNPass(DisableGVNLoadPRE));

        // Lowering prevents dead argument elimination later, so run IPO passes to
        // eliminate dead args.
        // TODO(dsm): Revisit running all standard opts here.
        // prepopulateModulePassManager() works fine for all working apps, but
        // crashes the compiler on 433.milc (which doesn't work anyway).
        MPM.add(createIPSCCPPass());          // IP SCCP
        MPM.add(createCFGSimplificationPass()); // Clean up after IP SCCP
        MPM.add(createPostOrderFunctionAttrsLegacyPass());  // Identify readnone
        MPM.add(createReversePostOrderFunctionAttrsPass()); // Identify readnone
        MPM.add(createGlobalOptimizerPass()); // Optimize out global vars
        // Promote any localized global vars.
        MPM.add(createPromoteMemoryToRegisterPass());
        MPM.add(createDeadArgEliminationPass()); // Dead argument elimination
    ...
    if (ParallelLevel != 4) {
      ...
      MPM.add(createSpatialHintsPass());
      MPM.add(createLowerTapirToSwarmPass());
      // Handle outlined methods that were tagged AlwaysInline
