...
Error BitcodeReader::parseFunctionBody(Function *F) {
  ...
  while (true) {
    ...
    case bitc::FUNC_CODE_INST_UNREACHABLE: // UNREACHABLE
      ...
      InstructionList.push_back(I);
      break;
    case bitc::FUNC_CODE_INST_SDETACH: { // SDETACH: [opval, bb#, bb#, sub, super]
      // TODO(eforde): teach BitcodeReader to read SDetach with hints
      // TODO(victory): teach BitcodeReader ti read SDetach with relative timestamp
      // TODO(victory): teach BitcodeWriter to read SDetach with domain token
      unsigned Idx = 0;
      if (Record.size() != 5)
        return error("Invalid record");

      Value *Timestamp;
      if (getValueTypePair(Record, Idx, NextValueNo, Timestamp))
        return error("Invalid record");

      BasicBlock *Detached = getBasicBlock(Record[Idx++]);
      if (!Detached)
        return error("Invalid record");

      BasicBlock *Continue = getBasicBlock(Record[Idx++]);
      if (!Continue)
        return error("Invalid record");

      I = SDetachInst::Create(Timestamp, Detached, Continue);

      cast<SDetachInst>(I)->setSubdomain(Record[Idx++]);
      cast<SDetachInst>(I)->setSuperdomain(Record[Idx++]);
      ...
      case bitc::FUNC_CODE_INST_SREATTACH: { // SREATTACH: [bb#]
      if (Record.size() != 1)
        return error("Invalid record");
      ...
      I = SReattachInst::Create(DetachContinue);
      InstructionList.push_back(I);
      break;
    }
