...
void ModuleBitcodeWriter::writeInstruction(const Instruction &I,
  ...
  case Instruction::SDetach:
    {
      // TODO(eforde): teach BitcodeWriter to write SDetach with hint
      // TODO(victory): teach BitcodeWriter to write SDetach with relative timestamp
      // TODO(victory): teach BitcodeWriter to write SDetach with domain token
      Code = bitc::FUNC_CODE_INST_SDETACH;
      const SDetachInst &DI = cast<SDetachInst>(I);
      pushValueAndType(DI.getTimestamp(), InstID, Vals);
      Vals.push_back(VE.getValueID(DI.getSuccessor(0)));
      Vals.push_back(VE.getValueID(DI.getSuccessor(1)));
      Vals.push_back(DI.isSubdomain());
      Vals.push_back(DI.isSuperdomain());
    }
    break;
  case Instruction::SReattach:
    {
      Code = bitc::FUNC_CODE_INST_SREATTACH;
