...
//===----------------------------------------------------------------------===//
//                        SDetachInst Implementation
//===----------------------------------------------------------------------===//

void SDetachInst::AssertOK() {
  Type *TimestampType = getTimestamp()->getType();
  assert(!hasTimestamp() || TimestampType->isIntegerTy() &&
         "Timestamps must be integers!");
  assert(!hasTimestamp() ||
         (TimestampType->getIntegerBitWidth() <= 64 &&
          "Timestamps cannot be more than 64 bits!"));

  assert(Op<1>()->getType()->isTokenTy() &&
         "Domain should be token (e.g., from deepen)");

  assert(getHint());
  Type *HintType = getHint()->getType();
  assert((!hasHint() ||
         (HintType->isIntegerTy() && HintType->getIntegerBitWidth() <= 64)) &&
         "Hint must be ConstantTokenNone or integer of no more than 64 bits!");

  assert(hasHint() + isNoHint() + isSameHint() == 1);
  assert(isSubdomain() + isSuperdomain() <= 1);

  Value *Condition = getSameHintCondition();
  assert((!isSameHint() || isa<ConstantTokenNone>(Condition) ||
          Condition->getType()->getIntegerBitWidth() == 1) &&
         "Either SAMEHINT is unconditional (ConstantTokenNone) or "
         "its condition is based on a 1-bit boolean.");
}

SDetachInst::SDetachInst(Value *Timestamp, Value *Domain, Value *Hint,
                         BasicBlock *Detached, BasicBlock *Continue,
                         Instruction *InsertBefore)
    : DetachInst(Type::getVoidTy(Detached->getContext()),
                 Instruction::SDetach,
                 OperandTraits<SDetachInst>::op_end(this) - 5, 5,
                 InsertBefore) {
  Op<-1>() = Detached;
  Op<-2>() = Continue;
  Op<0>() = Timestamp;
...
SDetachInst::SDetachInst(const SDetachInst &DI)
    : DetachInst(Type::getVoidTy(DI.getContext()), Instruction::SDetach,
                 OperandTraits<SDetachInst>::op_end(this) -
                 DI.getNumOperands(),
                 DI.getNumOperands()) {
  Op<0>() = DI.Op<0>();
  Op<1>() = DI.Op<1>();
...
Value *SDetachInst::getDomain() {
  Value *Domain = Op<1>();
  if (Domain == ConstantTokenNone::get(this->getContext())) {
    return nullptr;
  } else {
    assert(cast<CallInst>(Domain)->getCalledValue()->getName().equals("llvm.deepen"));
    return Domain;
  }
}

enum SDetachInstFlags {
                      SUBDOMAIN = 1 << 0,
                      SUPERDOMAIN = 1 << 1,
                      DOMAIN_MASK = SUBDOMAIN | SUPERDOMAIN,
                      RELTIMESTAMP = 1 << 2,
                      SAMEHINT = 1 << 3,
                     };

bool SDetachInst::isSubdomain() const {
  return getSubclassDataFromInstruction() & SUBDOMAIN;
}

void SDetachInst::setSubdomain(bool sub) {
  setInstructionSubclassData((getSubclassDataFromInstruction() & ~DOMAIN_MASK) |
                             (sub ? SUBDOMAIN :
                              isSuperdomain() ? SUPERDOMAIN : 0));
}

bool SDetachInst::isSuperdomain() const {
  return getSubclassDataFromInstruction() & SUPERDOMAIN;
}

void SDetachInst::setSuperdomain(bool super) {
  setInstructionSubclassData((getSubclassDataFromInstruction() & ~DOMAIN_MASK) |
                             (super ? SUPERDOMAIN :
                              isSubdomain() ? SUBDOMAIN : 0));
}

bool SDetachInst::isRelativeTimestamp() const {
  return getSubclassDataFromInstruction() & RELTIMESTAMP;
}

void SDetachInst::setRelativeTimestamp(bool isTimestampRel) {
  setInstructionSubclassData((getSubclassDataFromInstruction() & ~RELTIMESTAMP) |
                             (isTimestampRel ? RELTIMESTAMP : 0));
}

bool SDetachInst::isSameHint() const {
  return getSubclassDataFromInstruction() & SAMEHINT;
}

void SDetachInst::setIsSameHint(bool sameHint) {
  setInstructionSubclassData((getSubclassDataFromInstruction() & ~SAMEHINT) |
                             (sameHint ? SAMEHINT : 0));
  if (sameHint) Op<2>() = ConstantTokenNone::get(getTimestamp()->getContext());
  AssertOK();
}

const Value *SDetachInst::getSameHintCondition() const {
  // We reuse the Hint operand to hold the SAMEHINT condition, when applicable
  return Op<2>();
}

void SDetachInst::setSameHintCondition(Value *Condition) {
  if (!Condition) {
    setIsSameHint(false);
...
//===----------------------------------------------------------------------===//
//                      SReattachInst Implementation
//===----------------------------------------------------------------------===//

void SReattachInst::AssertOK() {
}

SReattachInst::SReattachInst(BasicBlock *DetachContinue,
                             Instruction *InsertBefore)
    : ReattachInst(Type::getVoidTy(DetachContinue->getContext()),
                   Instruction::SReattach,
                   OperandTraits<SReattachInst>::op_end(this) - 1, 1,
                   InsertBefore) {
  Op<-1>() = DetachContinue;
#ifndef NDEBUG
  AssertOK();
...
SReattachInst::SReattachInst(const SReattachInst &RI)
    : ReattachInst(Type::getVoidTy(RI.getContext()), Instruction::SReattach,
                   OperandTraits<SReattachInst>::op_end(this) -
                   RI.getNumOperands(),
                   RI.getNumOperands()) {
  Op<-1>() = RI.Op<-1>();
  assert(RI.getNumOperands() == 1 && "SReattach must have 1 operands!");
  SubclassOptionalData = RI.SubclassOptionalData;
}
...
DetachInst::DetachInst(const DetachInst &DI)
    : TerminatorInst(Type::getVoidTy(DI.getContext()), Instruction::Detach,
                     ...
                     DI.getNumOperands(),
                     DI.getNumOperands()) {
  assert(DI.getOpcode() != Instruction::SDetach &&
         "sdetach cannot be copied as detach");
  Op<-1>() = DI.Op<-1>();
  Op<-2>() = DI.Op<-2>();
  Op<-3>() = DI.Op<-3>();
...
SDetachInst *SDetachInst::cloneImpl() const {
  return new(getNumOperands()) SDetachInst(*this);
}

SReattachInst *SReattachInst::cloneImpl() const {
  return new(getNumOperands()) SReattachInst(*this);
}
