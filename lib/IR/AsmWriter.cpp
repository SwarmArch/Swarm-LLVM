...
static void PrintCallingConv(unsigned cc, raw_ostream &Out) {
  ...
  case CallingConv::X86_64_Swarm:  Out << "x86_64_swarmcc"; break;
  }
}
...
void AssemblyWriter::printInstruction(const Instruction &I) {
  ...
  if (isa<BranchInst>(I) && cast<BranchInst>(I).isConditional()) {
    ...
    if (DI.isRelativeTimestamp())
      Out << "relativetimestamp ";
    if (DI.isNoHint())
      Out << "nohint ";
    if (DI.isSameHint())
      Out << "samehint ";
    writeOperand(DI.getTimestamp(), true);
    if (const Value *Domain = DI.getDomain()) {
      Out << " within ";
      writeOperand(Domain, true);
    }
    Out << ", ";
    if (DI.hasHint()) {
      writeOperand(DI.getHint(), true);
      Out << ", ";
    }
    writeOperand(DI.getDetached(), true);
    Out << ", ";
    writeOperand(DI.getContinue(), true);
  } else if (isa<SReattachInst>(I)) {
    // Special case reattach instruction to get formatting nice and correct
    const SReattachInst &RI(cast<SReattachInst>(I));
    Out << " ";
