...
bool LLParser::ParseOptionalCallingConv(unsigned &CC) {
  ...
  case lltok::kw_x86_64_swarmcc: CC = CallingConv::X86_64_Swarm; break;
  case lltok::kw_cc: {
      Lex.Lex();
...
int LLParser::ParseInstruction(Instruction *&Inst, BasicBlock *BB,
  ...
  case lltok::kw_sdetach:     return ParseSDetach(Inst, PFS);
  case lltok::kw_sreattach:   return ParseSReattach(Inst, PFS);
...
/// ParseSDetach
///   ::= 'sdetach' 'subdomain'? 'superdomain'? 'relativetimestamp'? 'nohint'? 'samehint'?
///       TypeAndValue ['within' TypeAndValue] ',' TypeAndValue ',' TypeAndValue
///   ::= 'sdetach' 'subdomain'? 'superdomain'? 'relativetimestamp'?
///       TypeAndValue ['within' TypeAndValue] ',' TypeAndValue ',' TypeAndValue ',' TypeAndValue
bool LLParser::ParseSDetach(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc, Loc2;
  Value *Hint, *Timestamp;
  BasicBlock *Detached, *Continue;

  bool isSub = false;
  if (Lex.getKind() == lltok::kw_subdomain) {
    isSub = true;
    Lex.Lex();
  }

  bool isSuper = false;
  if (Lex.getKind() == lltok::kw_superdomain) {
    isSuper = true;
    Lex.Lex();
  }

  bool isRelativeTimestamp = false;
  if (Lex.getKind() == lltok::kw_relativetimestamp) {
    isRelativeTimestamp = true;
    Lex.Lex();
  }

  bool isNoHint = false;
  if (Lex.getKind() == lltok::kw_nohint) {
    isNoHint = true;
    Lex.Lex();
  }

  bool isSameHint = false;
  if (Lex.getKind() == lltok::kw_samehint) {
    isSameHint = true;
    Lex.Lex();
  }

  if (ParseTypeAndValue(Timestamp, Loc, PFS)) return true;
  if (!Timestamp->getType()->isIntegerTy())
    return Error(Loc, "detach timestamp must have integer type");

  Value *Domain = nullptr;
  if (Lex.getKind() == lltok::kw_within) {
    Lex.Lex();

    if (ParseTypeAndValue(Domain, Loc, PFS)) return true;
    if (!Domain->getType()->isTokenTy())
      return Error(Loc, "detach domain must have token type");
  }

  if (ParseToken(lltok::comma, "expected ',' after detach timestamp"))
    return true;

  if (!isNoHint && !isSameHint) {
    if (ParseTypeAndValue(Hint, Loc, PFS)) return true;
    if (!Hint->getType()->isIntegerTy())
      return Error(Loc, "detach hint must have integer type");
    if (ParseToken(lltok::comma, "expected ',' after detach hint"))
      return true;
  }

  if (ParseTypeAndBasicBlock(Detached, Loc, PFS) ||
      ParseToken(lltok::comma, "expected ',' after detached destination") ||
      ParseTypeAndBasicBlock(Continue, Loc2, PFS))
      return true;

  Inst = SDetachInst::Create(Timestamp, Detached, Continue);
  auto DI = cast<SDetachInst>(Inst);
  DI->setSubdomain(isSub);
  DI->setSuperdomain(isSuper);
  DI->setRelativeTimestamp(isRelativeTimestamp);
  DI->setDomain(Domain);
  if (isNoHint)
    DI->setHint(nullptr);
  DI->setIsSameHint(isSameHint);

  return false;
}

/// ParseSReattach
///   ::= 'sreattach' TypeAndValue
bool LLParser::ParseSReattach(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc;
  BasicBlock *Op;
