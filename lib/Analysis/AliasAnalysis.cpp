...
ModRefInfo AAResults::getModRefInfo(Instruction *I, ImmutableCallSite Call) {
  ...
  if (auto CS = ImmutableCallSite(I)) {
    ...
    BasicBlock *Continue = D->getContinue();
    bool MustHaveNoMatchingReattaches = false;
    if (Continue->getSinglePredecessor()) {
      auto B = dyn_cast<BranchInst>(Continue->getTerminator());
      if (B && B->isUnconditional()) {
        // The continue edge must have been split.
        Continue = Continue->getSingleSuccessor();
        assert(Continue);
      } else {
        MustHaveNoMatchingReattaches = true;
      }
    }
    ...
    while (!WorkList.empty()) {
      ...
      assert(!MustHaveNoMatchingReattaches || !isa<ReattachInst>(T));
