...
protected:
  // victory: These "forward" constructors are used only by SDetachInst
  DetachInst(Type *Ty, Instruction::TermOps iType,
             Use *Ops, unsigned NumOps,
             Instruction *InsertBefore = nullptr)
    : TerminatorInst(Ty, iType, Ops, NumOps, InsertBefore) {}
  DetachInst(Type *Ty, Instruction::TermOps iType,
                 Use *Ops, unsigned NumOps, BasicBlock *InsertAtEnd)
    : TerminatorInst(Ty, iType, Ops, NumOps, InsertAtEnd) {}
...
public:
  ...
  Value *getSyncRegion() const {
    assert(getOpcode() != Instruction::SDetach &&
           "sdetach has no syncregion");
    return Op<-3>();
  }
  ...
  static inline bool classof(const Instruction *I) {
    return (I->getOpcode() == Instruction::Detach)
            || (I->getOpcode() == Instruction::SDetach);
  }
  ...
  inline void setDetached(BasicBlock *Detached) { setSuccessor(0, Detached); }
  inline void setContinue(BasicBlock *Continue) { setSuccessor(1, Continue); }
private:
  friend TerminatorInst;
...
protected:
  // victory: These "forward" constructors are used only by SReattachInst
  ReattachInst(Type *Ty, Instruction::TermOps iType,
               Use *Ops, unsigned NumOps,
               Instruction *InsertBefore = nullptr)
    : TerminatorInst(Ty, iType, Ops, NumOps, InsertBefore) {}
  ReattachInst(Type *Ty, Instruction::TermOps iType,
...
public:
  ...
  Value *getSyncRegion() const {
    assert(getOpcode() != Instruction::SReattach &&
           "sreattach has no syncregion");
    return Op<-2>();
  }
  ...
  static inline bool classof(const Instruction *I) {
    return (I->getOpcode() == Instruction::Reattach)
            || (I->getOpcode() == Instruction::SReattach);
  }
  static inline bool classof(const Value *V) {
    return isa<Instruction>(V) && classof(cast<Instruction>(V));
  }
...
//===---------------------------------------------------------------------------
/// SDetachInst - Swarm detach instruction
///
class SDetachInst : public DetachInst {
  SDetachInst(const SDetachInst &DI);
  void AssertOK();
  // SDetachInst constructors (where {D, C} are blocks and T is a timestamp):
  // SDetachInst(V *T, V *D, V *H, BB *D, BB *C)         - 'detach T, D, C'
  // SDetachInst(V *T, V *D, V *H, BB *D, BB *C Inst *I) - 'detach T, D, C', insert before I
  // SDetachInst(V *T, V *D, V *H, BB *D, BB *C BB *I)   - 'detach T, D, C', insert at end
  SDetachInst(Value *Timestamp, Value *Domain, Value *Hint,
              BasicBlock *Detached, BasicBlock *Continue,
              Instruction *InsertBefore = nullptr);
  SDetachInst(Value *Timestamp, Value *Domain, Value *Hint,
              BasicBlock *Detached, BasicBlock *Continue,
              BasicBlock *InsertAtEnd);
protected:
  // Note: Instruction needs to be a friend here to call cloneImpl.
  friend class Instruction;
  SDetachInst *cloneImpl() const;

public:
  static SDetachInst *Create(BasicBlock *Detached, BasicBlock *Continue,
                             Instruction *InsertBefore = nullptr) {
    Value *Timestamp = ConstantTokenNone::get(Detached->getContext());
    Value *Hint = ConstantTokenNone::get(Detached->getContext());
    return SDetachInst::Create(Timestamp, Hint, Detached, Continue, InsertBefore);
  }
  static SDetachInst *Create(Value *Timestamp,
                             BasicBlock *Detached, BasicBlock *Continue,
                             Instruction *InsertBefore = nullptr) {
    Value *Hint = ConstantTokenNone::get(Timestamp->getContext());
    return SDetachInst::Create(Timestamp, Hint, Detached, Continue, InsertBefore);
  }
  static SDetachInst *Create(Value *Timestamp,
                             BasicBlock *Detached, BasicBlock *Continue,
                             BasicBlock *InsertAtEnd) {
    Value *Hint = ConstantTokenNone::get(Timestamp->getContext());
    return SDetachInst::Create(Timestamp, Hint, Detached, Continue, InsertAtEnd);
  }
  static SDetachInst *Create(Value *Timestamp, Value *Hint,
                             BasicBlock *Detached, BasicBlock *Continue,
                             Instruction *InsertBefore = nullptr) {
    Value *Domain = ConstantTokenNone::get(Timestamp->getContext());
    return new(5) SDetachInst(Timestamp, Domain, Hint, Detached, Continue, InsertBefore);
  }
  static SDetachInst *Create(Value *Timestamp, Value *Hint,
                             BasicBlock *Detached, BasicBlock *Continue,
                             BasicBlock *InsertAtEnd) {
    Value *Domain = ConstantTokenNone::get(Timestamp->getContext());
    return new(5) SDetachInst(Timestamp, Domain, Hint, Detached, Continue, InsertAtEnd);
  }
  ...
  */
  ...
  // Methods for support type inquiry through isa, cast, and dyn_cast:
  static inline bool classof(const Instruction *I) {
    return (I->getOpcode() == Instruction::SDetach);
  }
  static inline bool classof(const Value *V) {
    return isa<Instruction>(V) && classof(cast<Instruction>(V));
  }
  ...
  */
  ...
  const Value *getTimestamp() const { return Op<0>(); }
  Value *getTimestamp() { return Op<0>(); }
  inline void setTimestamp(Value *V) {
    Op<0>() = V;
  }
  bool hasTimestamp() const {
    return !isa<ConstantTokenNone>(getTimestamp());
  }

  inline const Value *getDomain() const {
    return const_cast<SDetachInst *>(this)->getDomain();
  }
  Value *getDomain();
  inline void setDomain(Value *Domain = nullptr) {
    Op<1>() = Domain ? Domain : ConstantTokenNone::get(this->getContext());
  }

  // We use Op<1>() for multiple Hint-related purposes:
  // 1) An integer Value when the task will enqueue with a specific hint,
  // 2) A ConstantTokenNone indicating an
  //    always-no-hint or always-same-hint enqueue, or
  // 3) A boolean condition Value indicating whether to perform
  //    a no-hint or same-hint enqueue
  const Value *getHint() const { return Op<2>(); }
  Value *getHint() { return Op<2>(); }
  bool hasHint() const {
    return !isSameHint() && !isa<ConstantTokenNone>(getHint());
  }
  void setHint(Value *V) {
    if (!V)
      Op<2>() = ConstantTokenNone::get(this->getContext());
    else
      Op<2>() = V;
    setIsSameHint(false);
  }
  bool isSameHint() const;
  void setIsSameHint(bool);
  bool isNoHint() const { return !hasHint() && !isSameHint(); }
  // Use these methods when the task needs to be conditionally spawned
  // as either SAMEHINT (when the condition is true) or NOHINT (otherwise).
  const Value *getSameHintCondition() const;
  Value *getSameHintCondition() {
    const SDetachInst *CThis = static_cast<const SDetachInst *>(this);
    return const_cast<Value *>(CThis->getSameHintCondition());
  }
  bool hasSameHintCondition() const {
    return isSameHint() && !isa<ConstantTokenNone>(getSameHintCondition());
  }
  void setSameHintCondition(Value *Condition);

  bool isSubdomain() const ;
  void setSubdomain(bool sub) ;
  bool isSuperdomain() const ;
  void setSuperdomain(bool super) ;
  bool isRelativeTimestamp() const ;
  void setRelativeTimestamp(bool isRelativeTimestamp) ;
...
template <>
struct OperandTraits<SDetachInst> : public VariadicOperandTraits<SDetachInst, 1> {
};

DEFINE_TRANSPARENT_OPERAND_ACCESSORS(SDetachInst, Value)
...
//===---------------------------------------------------------------------------
/// SReattachInst - SReattach instruction.  This instruction terminates
/// a subCFG and has no successors.  The DetachContinue field
/// maintains the continue block after the sdetach instruction
/// corresponding to this reattach.
///
class SReattachInst : public ReattachInst {
  SReattachInst(const SReattachInst &RI);
  void AssertOK();
  // SReattachInst constructors (where C is a block):
  // SReattachInst(BB *C)          - 'reattach C'
  // SReattachInst(BB *C, Inst *I) - 'reattach C', insert before I
  // SReattachInst(BB *C, BB *I)   - 'reattach C', insert at end
  explicit SReattachInst(BasicBlock *DetachContinue,
                         Instruction *InsertBefore = nullptr);
  SReattachInst(BasicBlock *DetachContinue,
               BasicBlock *InsertAtEnd);
protected:
  // Note: Instruction needs to be a friend here to call cloneImpl.
  friend class Instruction;
  SReattachInst *cloneImpl() const;

public:
  static SReattachInst *Create(BasicBlock *DetachContinue,
                               Instruction *InsertBefore = nullptr) {
    return new(1) SReattachInst(DetachContinue, InsertBefore);
  }

  static SReattachInst *Create(BasicBlock *DetachContinue,
                               BasicBlock *InsertAtEnd) {
    return new(1) SReattachInst(DetachContinue, InsertAtEnd);
  }
  ...
  */
  ...
  // Methods for support type inquiry through isa, cast, and dyn_cast:
  static inline bool classof(const Instruction *I) {
    return I->getOpcode() == Instruction::SReattach;
  }
  static inline bool classof(const Value *V) {
    return isa<Instruction>(V) && classof(cast<Instruction>(V));
  }
...
template <>
struct OperandTraits<SReattachInst> : public VariadicOperandTraits<SReattachInst, 1> {
};

DEFINE_TRANSPARENT_OPERAND_ACCESSORS(SReattachInst, Value)
