...
public:
  ...
  /// \brief Create a Swarm detach instruction, 'detach Timestamp, Detached,
  // Continue'.
  SDetachInst *CreateSDetach(Value *Timestamp, BasicBlock *Detached, BasicBlock *Continue,
                             MDNode *BranchWeights = nullptr) {
    return Insert(addBranchMetadata(SDetachInst::Create(Timestamp,
                                                        Detached, Continue),
                                    BranchWeights, nullptr));
  }

  /// \brief Create a Swarm detach instruction, 'detach Detached, Continue'.
  SDetachInst *CreateSDetach(BasicBlock *Detached, BasicBlock *Continue,
                             MDNode *BranchWeights = nullptr) {
    return Insert(addBranchMetadata(SDetachInst::Create(Detached, Continue),
                                    BranchWeights, nullptr));
  }

  /// \brief Create a Swarm reattach instruction, 'sreattach DetachContinue'.
  SReattachInst *CreateSReattach(BasicBlock *DetachContinue) {
    return Insert(SReattachInst::Create(DetachContinue));
  }
